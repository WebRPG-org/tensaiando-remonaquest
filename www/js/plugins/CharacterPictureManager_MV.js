...
(function() {
    ...
    // -------------------------------------------------------------------------
    // MV compatibility layer (no functionality changes)
    //  - Provides minimal PluginManagerEx and Sprite_Clickable when running on MV
    //  - Keeps MZ behavior when those classes already exist
    // -------------------------------------------------------------------------
    (function() {
        const globalObj = (function() { return this || (0, eval)('this'); })();

        // PluginManagerEx is provided by PluginCommonBase on MZ.
        // MV doesn't have it by default, so we polyfill only what this plugin uses.
        if (typeof globalObj.PluginManagerEx === 'undefined') {
            globalObj.PluginManagerEx = {};

            globalObj.PluginManagerEx.throwError = function(message) {
                throw new Error(message);
            };

            globalObj.PluginManagerEx.findClassName = function(instance) {
                return instance && instance.constructor ? instance.constructor.name : '';
            };

            globalObj.PluginManagerEx.findMetaValue = function(data, metaName) {
                if (!data) return '';
                // MV stores meta in .meta (after database load)
                if (data.meta && data.meta.hasOwnProperty(metaName)) {
                    return data.meta[metaName];
                }
                return '';
            };

            // Parse MZ-like structured parameters (JSON strings) on MV as well.
            globalObj.PluginManagerEx.createParameter = function(script) {
                const pluginName = (function() {
                    const src = script && script.src ? script.src : '';
                    const m = src.match(/([^\/\\]+)\.js$/);
                    return m ? m[1] : 'CharacterPictureManager';
                })();

                const raw = PluginManager.parameters(pluginName) || {};

                const coercePrimitive = function(value) {
                ...
                var parse = function(value) {
                    ...
                    // Try JSON first (supports nested JSON-as-string used by MZ struct params)
                    ...
                    return coercePrimitive(v);
                };

                const params = {};
                Object.keys(raw).forEach(function(key) {
                    params[key] = parse(raw[key]);
                });
                return params;
            };
        }

        // Sprite_Clickable exists on MZ. MV doesn't have it, so we polyfill a compatible one.
        if (typeof globalObj.Sprite_Clickable === 'undefined') {
            ...
            Sprite_Clickable.prototype.isButtonTouched = function() {
                ...
                const pos = this._findLocalTouchPos();
                return pos.x >= 0 && pos.y >= 0 && pos.x < this.width && pos.y < this.height;
            };

            Sprite_Clickable.prototype._findLocalTouchPos = function() {
                const touchPos = new Point(TouchInput.x, TouchInput.y);
                const pos = this.worldTransform.applyInverse(touchPos);
                pos.x += this.width * (this.anchor ? this.anchor.x : 0);
                pos.y += this.height * (this.anchor ? this.anchor.y : 0);
                return pos;
            };

            globalObj.Sprite_Clickable = Sprite_Clickable;
        }

        // If APNG plugin is not present on MV, keep behavior: treat as normal picture.
        if (typeof SceneManager.tryLoadApngPicture !== 'function') {
            SceneManager.tryLoadApngPicture = function() {
                return null;
            };
        }
    })();
...
updatePictureFiles() {
  // 必要ならこの先で使うキャッシュもここで更新（後述）
  this._standPictures.forEach(picture => {
    picture.FileName = null;

    const list = picture.FileList;
    if (list && list.length) {
      for (let i = list.length - 1; i >= 0; i--) {
        if (this.setFileNameIfValid(list[i], picture)) break;
      }
    }
  ...
  return this._standPictures;
    ...
    Scene_Base.prototype.isStandPictureScene = function(sceneName) {
        return this._standSpriteScene && this._standSpriteScene.SceneName === sceneName;
    };
